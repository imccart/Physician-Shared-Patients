...
pacman::p_load(tidyverse, ggplot2, dplyr, lubridate, stringr, readxl, data.table, gdata, scales,
               stargazer, kableExtra, tidygraph, ggraph)
```
...
Now let's take a look at the network structure of PCP referrals to specialist, focusing on shared patients among physicians in Atlanta, GA. For this, we'll use the `tidygraph` package.

```{r echo=FALSE}
edge_small <- edge_data %>% filter()
  group_by(from) %>% mutate(pcp_count=n()) %>% ungroup() %>%
  top_n(10, wt=pcp_count) %>%
  select(from, to, weight)
  
edge.npis <- rbind(edge_small %>% select(npi=from), edge_small %>% select(npi=to))
node_small <- node_data %>% inner_join(edge.npis, by="npi") %>% select(name=npi, graduation=graduation_1)
